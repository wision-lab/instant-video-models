...
def process_video_groups(model, device, video_data, transform, epsilon):
    model.eval()
    ...
    for video_path, groups in video_data.items():
        for label, image_paths in groups:
            ...
            for image_path in image_paths:
                ...
                perturbed_image = iterative_fgsm_attack(model, image, true_tensor, epsilon=epsilon, iters=100, device=device)
...
def iterative_fgsm_attack(model, data, target, epsilon, iters=100, device=None):
...
def main(config_path, epsilon, decay):
    ...
    process_video_groups(model, device, test_data, test_transform, epsilon)
