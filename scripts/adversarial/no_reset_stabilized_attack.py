...
from stability.utils import add_hook_modules, invoke_on_values
import argparse
...
def load_images_from_videos(video_paths):
    ...
    for video_path in sorted(video_paths):
        ...
        for object_id, frames in object_groups.items():
            ...
            video_data.setdefault(video_path, []).append((object_id, sorted_image_paths))

    return video_data

def process_video_groups(model, device, video_data, transform, stabilizer_dict, epsilon):
    model.eval()
    ...
    for video_path, groups in video_data.items():
        for label, image_paths in groups:
            ...
            for image_path in image_paths:
                ...
                perturbed_image = iterative_fgsm_attack(model, image, true_tensor, epsilon=epsilon, iters=100, stabilizer_dict=stabilizer_dict, device=device)
            ...
            print(f"Ended {label} video in folder {video_path} with accuracy: {accuracy:.4f}")
...
def iterative_fgsm_attack(model, data, target, epsilon, iters=100, stabilizer_dict=None, device=None):
...
def main(config_path, epsilon, decay):
    ...
    config['stabilizers'][0]['module']['_target'] = 'stability.stabilizers.ema_stabilizer.EMAStabilizer'
    ...
    process_video_groups(model, device, test_data, test_transform, stabilizer_dict, epsilon)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run model on video data with configurable settings.")
    parser.add_argument("--config", type=str, required=True, help="Path to the configuration file.")
    parser.add_argument("--epsilon", type=float, required=True, help="Epsilon value for the attack.")
    parser.add_argument("--decay", type=float, required=True, help="Decay value for the stabilizer.")
